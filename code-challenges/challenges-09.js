...
const employeesBonus = (arr) => {
    function calculateBonus(workHours) {
        if (workHours > 8) {
            return 100;
        } else {
            return 50;
    ...
    function updateSalary(employee, bonus) {
        const currentSalary = Number(employee.salary.slice(0, -1)); 
        const updatedSalary = `${currentSalary + bonus}$`; 
        return {...employee, salary: updatedSalary}; 
    }
    function applyBonuses(employees) {
        return employees.map(employee => {
            const bonus = calculateBonus(employee.workHours);
            return updateSalary(employee, bonus);
        });
    }

    return applyBonuses(arr); 
}
// -------------------------------------------------------------------------------------------------------
