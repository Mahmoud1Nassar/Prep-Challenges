...
const recursionPattern = (int1, int2) => {
  ...
  const helper = (current, isIncreasing) => {
    result.push(current);

    if (!isIncreasing) {
      if (current - int2 > 0) {
        helper(current - int2, false);
      } else {
        helper(current - int2, true);
      }
    } else {
      if (current < int1) {
        helper(current + int2, true);
...
const filterLinks = (str) => {
  const hrefRegex = /href="http:\/\/([^"]+\.(com|org|net))"/;
...
const isPalindrome = (str) => {
  ...
  const filteredStr = str.replace(/[^a-z0-9]/g, "");
