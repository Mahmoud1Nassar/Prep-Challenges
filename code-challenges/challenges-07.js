...
const objLat = (obj) => {
    ...
    function cap(firstName) {
        let capFN = firstName.charAt(0).toUpperCase() + firstName.slice(1);
        return capFN;
    }
    return `my name is ${cap(obj.firstName)} ${obj.lastName} I am ${obj.age} YO, and I love ${obj.hobby}.`
};
// -------------------------------------------------------------------------------------------------------
...
const cvFormatter = (arr) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        if (arr[i].yearsOfExperience > 1) {
            ...
            tech = arr[i].tech;
            formatedArr.push({ fullName, tech });
        }
...
const applicationsStatics = (arr) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        if (((arr[i].firstName === null || arr[i].firstName === '') && (arr[i].lastName === null || arr[i].lastName === '')) || arr[i].yearsOfExperience <= 1) {
            ...
            const tech = arr[i].tech.toLowerCase();
            if (tech === 'python') {
                result.python_devs++;
            } else if (tech === 'js') {
...
// data.grades[forloop].classess[foorlop].classScores[calAvg]
//cal avg function => for loop to sum the grades after that get the length of the grades and divide them
