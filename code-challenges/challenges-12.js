...
// Required:
// 
// Given an array of values use filter to extract odd values.0
//  
// Input:
...
const cvsFiltration = (arr) => {
    return arr.filter(cv => cv.yearsOfExperience > 4 && cv.tech === "JS").map(cv => {
            const fullName = [cv.firstName, cv.LastName].filter(name => name !== null).join(' ');
            return {
                fullName: fullName,
                tech: cv.tech
            };
